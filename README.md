...
├── src/
│   ├── train.py               # cleans data, trains + compares 3 models, saves the best one
│   └── predict.py              # loads the saved model, predicts on new customer data (CLI)
├── models/                    # saved pipeline + decision threshold (generated by train.py)
├── reports/                   # model comparison table, feature importance, confusion matrices
